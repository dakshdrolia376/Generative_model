...
if __name__ == '__main__':
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(0, train_size, batch_size):
            ...
            c_continuous1 = xp.asarray(rnd_continuous(batch_size, 1), dtype=xp.float32)
            c_continuous2 = xp.asarray(rnd_continuous(batch_size, 1, mu = z_mu, std = z_sigma), dtype=xp.float32)
            c_continuous = xp.concatenate(( c_continuous1, c_continuous2), axis=1)
            zc = xp.concatenate((zs, c_categorical, c_continuous), axis=1)

            # Forward
            x_fake = generator(zc)
            y_fake, mi = discriminator(x_fake)
            shape = tuple([batch_size]) + im_shape

            x_real = xp.zeros(shape, dtype=xp.float32)
